...
TEST(HNSWTest, GoodRecallBeforeAndAfterSerialization) {
  ...
  hnsw::HierarchicalNSW<float>* alg_hnsw =
      new hnsw::HierarchicalNSW<float>(&space, max_elements, M, ef_construction);
...
TEST(HNSWTest, DeterministicQueries) {
  ...
  vectorlib::AlgorithmInterface<float>* alg_hnsw =
      new vectorlib::hnsw::HierarchicalNSW<float>(&space, 2 * n);
