...
inline uint32_t getChunkId(idx_t combinedId) { return static_cast<uint32_t>(combinedId >> 32); }
//---------------------------------------------------------------------------
...
std::vector<std::pair<DocumentID, double>> VectorEngine::search(
  ...
  std::priority_queue<std::pair<float, idx_t>> pq =
      hnsw_alg->searchKnn(chunk_embeddings[0].data(), num_results);
...
//---------------------------------------------------------------------------
uint32_t VectorEngine::getDocumentCount() { return doc_count; }
//---------------------------------------------------------------------------
