#ifndef FTS_L2SPACESPARSE_H
#define FTS_L2SPACESPARSE_H
...
class L2SqrSparseFunctor {
 public:
  // Constructor takes references to external objects
  L2SqrSparseFunctor(const std::vector<std::vector<float>>& document_to_vector,
...
// };  // namespace hnsw
//--------------------------------------------------------------------------------------------------
class L2SpaceSparse : public SpaceInterface<float> {
 public:
  explicit L2SpaceSparse(size_t dim, std::vector<std::vector<float>>& document_to_vector_,
 ...
 private:
  ...
  L2SqrSparseFunctor l2_distance_functor_;
