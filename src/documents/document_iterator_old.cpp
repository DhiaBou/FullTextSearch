#include "document_iterator_old.hpp"

#include <arrow/array.h>
#include <arrow/memory_pool.h>
#include <arrow/record_batch.h>

DocumentIteratorOld::DocumentIteratorOld(const std::string &folder_path) {
...
void DocumentIteratorOld::loadNextFile() {
  if (file_queue.empty()) {
    batch_reader.reset();
    arrow_reader.reset();
    return;
...
bool DocumentIteratorOld::loadNextBatch() {
  if (!batch_reader) {
    return false;
  }
...
bool DocumentIteratorOld::hasNext() {
  while (true) {
    if (!current_batch) {
      return false;
    }
...
std::shared_ptr<Document> DocumentIteratorOld::operator*() {
  int32_t length = 0;
...
void DocumentIteratorOld::operator++() {
  if (!hasNext()) {
...
void DocumentIteratorOld::operator++(int) { ++(*this); }